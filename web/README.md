...
**Ikona to żółty znak w kaflu, nie logo firmy.** Ikona ma mówić, KTÓRA to
...
Znak to ikona **„bug" z biblioteki [Lucide](https://lucide.dev), licencja ISC**
(atrybucja nieobowiązkowa). Zgodnie z regułą systemu projektowego (§5) znaki
wszystkich naszych aplikacji pochodzą wyłącznie z Lucide (siatka 24, kreska 2) —
dzięki temu mają jednakową grubość linii. Podmiana: pobierz nowy plik
z [lucide.dev](https://lucide.dev), zapisz jako `icon-mark.svg`, uruchom
`tools/make_icons.py` (generuje 4 kafle PNG).
